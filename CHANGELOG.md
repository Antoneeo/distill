...
reader.** Distill governed only the output side of the channel: what to keep when you
write. But an answer is generated by the same process that reads, and an answer begun
before the author's point is reconstructed argues with a strawman (a live failure, not a
hypothesis). Comprehending someone's text and compressing your own are the same
operation; the skill now says so.

### Added

- **§2 "Before the contract: the reading"**: restate the other's point in its strongest
  form, in thinking, before answering — cannot write it = not done reading. An
  alternative dies only by naming the fact that kills it, and stays on the map. Options
  that keep tying mean the question needs recompressing, not the answers. The unverified
  is declared, not hidden.
- **The persistence hook now carries both halves** in one line: reading first (restate,
  named-fact elimination, recompress the question), then writing (≤5 assertions serving
  the reader's next action, answer first, the §6 bans). For an autoregressive model a
  method is real only if it changes generation order, so the reading half is phrased as
  a sequence constraint on the thinking block.

### Changed

- The payload budget rises from <260 to <500 characters (~110 tokens per prompt) — a
  deliberate author decision: both sides of the channel in one payload. The battery now
  also asserts both halves are present, so a future edit cannot silently drop one.
- Shipped without a dogfood window, deliberately: the addition is injected text — no
  network, no filesystem, no blocking, reversible at the next release through the 0.6.0
  update lane — and the phase-1 gate only measures output form, so waiting would have
  bought impressions, not data.
